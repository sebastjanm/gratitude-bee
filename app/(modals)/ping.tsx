// This file was created by the assistant to display ping message details.
//
// Changes:
// - Created to handle incoming 'ping_sent' notifications.
// - Parses the standardized `payload` object from route params.
// - Displays ping details and provides an acknowledgement button.

import React, { useState } from 'react';
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, ActivityIndicator } from 'react-native';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { X, Radio, MessageSquare } from 'lucide-react-native';
...
export default function PingDetailScreen() {
  ...
  if (!payload) {
    return (
      <View style={styles.container}>
        <Text>No ping details found.</Text>
      </View>
    );
  }
  ...
  const handleAcknowledge = async () => {
    ...
    setLoading(true);
    try {
      // Re-using the generic 'send-thank-you' function for acknowledgement
      ...
      Alert.alert('Success', 'You have acknowledged the ping.');
    } catch (error) {
      console.error('Error acknowledging ping:', error);
      Alert.alert('Error', 'Could not acknowledge the ping.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          <View style={styles.iconContainer}>
            <Radio size={40} color="#3498db" />
          </View>
          <Text style={styles.title}>{title || 'You have a Ping!'}</Text>
        ...
        <Text style={styles.footerText}>
          Let your partner know you've seen their message.
        </Text>
      </ScrollView>
    </View>
  );
