-- This migration updates the get_user_analytics function to include WISDOM, PING, and DONT_PANIC events
-- in the category breakdown and insights calculations.
...
begin
  ...
  from (
    select
      initcap(replace(coalesce(e.content->>'category_id', e.event_type::text), '_', ' ')) as name,
      ...
      case coalesce(e.content->>'category_id', e.event_type::text)
        ...
        when 'WISDOM' then '#9B59B6'
        when 'PING' then '#3B82F6'
        when 'DONT_PANIC' then '#6366F1'
        else '#6B7280'
      end as color
    from public.events e
    where e.event_type in ('APPRECIATION', 'HORNET', 'WISDOM', 'PING', 'DONT_PANIC')
    and e.created_at >= start_date and e.created_at < end_date
    and (e.sender_id = p_user_id or e.receiver_id = p_user_id)
  ...
  if p_period = 'today' then
    with
      ...
      sent_today as (
        select * from events_today where sender_id = p_user_id
      ),
      first_event as (
      ...
      category_breakdown as (
        select initcap(replace(coalesce(content->>'category_id', event_type::text), '_', ' ')) as category, count(*) as cnt
        from sent_today
        where event_type in ('APPRECIATION', 'HORNET', 'WISDOM', 'PING', 'DONT_PANIC')
        group by 1
        order by cnt desc
      )
    select jsonb_build_object(
      'streak_status', (
        select case when count(*) > 0 then 'On a streak!' else 'No streak today.' end 
        from sent_today where event_type = 'APPRECIATION'
      ),
    ...
    with
      ...
      partner_events as (
        select * from all_events where sender_id = v_partner_id
      ),
      most_active_day as (
      ...
      favorite_category as (
        select initcap(replace(coalesce(content->>'category_id', event_type::text), '_', ' ')) as category_name
        from sent_events
        where event_type in ('APPRECIATION', 'HORNET', 'WISDOM', 'PING', 'DONT_PANIC')
      ...
      balance as (
        select
          greatest(count(case when sender_id = p_user_id then 1 end), 1) as sent_count,
          greatest(count(case when receiver_id = p_user_id then 1 end), 1) as received_count
        from all_events
        where event_type in ('APPRECIATION', 'WISDOM') -- Only positive, non-utility events
      )
    select
      jsonb_build_object(
        'most_active_day', (select day_name from most_active_day),
        'favorite_category', (select category_name from favorite_category),
        'partner_favorite_category', (select category_name from partner_favorite_category),
        'balance_score', round((select least(sent_count::decimal / received_count, received_count::decimal / sent_count) from balance), 2)
      )
    into insights_json;
  end if;
