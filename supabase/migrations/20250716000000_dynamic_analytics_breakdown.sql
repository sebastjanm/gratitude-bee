-- This migration completely rewrites the analytics function to support dynamic time-based breakdowns.
-- It introduces hourly, daily, weekly, and monthly views based on the selected period.
...
begin
  ...
  case p_period
    when 'today' then
      ...
      breakdown_title := 'Hourly Breakdown (Last 24h)';
    when 'week' then
      start_date := date_trunc('week', now());
      end_date := start_date + interval '1 week';
      breakdown_title := 'Daily Breakdown';
    when 'month' then
      start_date := date_trunc('month', now());
      end_date := start_date + interval '1 month';
      breakdown_title := 'Weekly Breakdown';
    else -- 'all'
  ...
  if p_period = 'today' then
    ...
    from (
      select
        ...
        coalesce(sum(case when event_type = 'HORNET' then 1 else 0 end), 0)::int as negative,
        coalesce(count(e.id), 0)::int as total
      from generate_series(start_date, end_date - interval '1 day', '1 week') as d
    ...
    from (
      ...
      from generate_series(
        (select date_trunc('month', min(created_at)) from public.events),
        now(),
        '1 month'
      ) as m
  ...
  -- === 4. Insights (Uses the period filter) ===
  with
  all_events as (
    select * from public.events
    where created_at >= start_date and created_at < end_date
